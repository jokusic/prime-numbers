...
*> File: primes3, re-engineered code to allow the user to enter numbers from the keyboard.
...
working-storage section.
    ...
    77  eof pic x(4).
    77  temp pic s9(9).
    77  in-new pic s9(9) value 1.
    77  new-line pic x(1) value x'0d'.
...
procedure division.
    ...
    display "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*".
    display "Enter numbers from a file or keyboard (F/K)?".
    read standard-input into user-input.
    
    *> Condition if user does not select file or keyboard,
    *> tell them they must enter in the proper requested information
    if in-c is not equal to "F" and "f" and "K" and "k"
    then
        display "Improper Input! Please enter a 'F' or 'K' next time!"
        stop run
    end-if.
    
    
    *> Conidition if user selects to input numbers through a file
    if in-c = "F" or 'f'
    then
        *> Grabs file name to open and write to
        display "Name of file to open (with extension):"
        read standard-input into input-file-name
        display "Name of file to write to (with extension):"
        read standard-input into output-file-name
        ...
        *> Closes all files that were opened
        display "COMPLETED"
        close input-file, output-file, standard-input
    end-if.
    
    *> Condition if user requests to input numbers through the keyboard
    if in-c = "K" or "k" 
        open output standard-output
        
        *> Sets end of loop condition
        perform until in-new = 0
        
            *> Reads in user input
            display "Type 0 to exit"
            read standard-input into in-new
            move in-new to num  
                  
            if in-new is not equal to 0
            then
                *> Checks for illegal input
                if num <= 1
                then
                    move in-new to out-n
                    write stdout-record from title-line after advancing 0 lines
                    write stdout-record from under-line after advancing 1 line
                    write stdout-record from error-mess after advancing 1 line
                    write stdout-record from new-line
                end-if
                    ...
                    *> Checks to see if the number is divisble by another number
                    perform until (remain >= num or temp = num)
                        ...
                        then
                            ...
                            move in-new to out-n-2
                            write stdout-record from title-line after advancing 0 lines
                            write stdout-record from under-line after advancing 1 line
                            write stdout-record from not-a-prime-line after advancing 1 line
                            write stdout-record from new-line
                            *> Exit loop condition
                            move num to temp
                        end-if
                ...
                *> Any number > 1 and < 4 is considered prime  
                if (remain >= num) or (num < 4 and num > 1)
                then
                    move in-new to out-n-3
                    write stdout-record from title-line after advancing 0 lines
                    write stdout-record from under-line after advancing 1 line
                    write stdout-record from prime-line after advancing 1 line
                    write stdout-record from new-line
                end-if
            end-if
        end-perform  
